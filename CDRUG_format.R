...
drug.map2 <- drug.map[complete.cases(drug.map[ , c("CCLE", "CGP")]), c("CCLE", "CGP"), drop=FALSE]
...
## boxplot
xx <- -log10(ic50.cgp / 10^6)
oo <- order(apply(xx, 2, median, na.rm=TRUE), apply(xx, 2, IQR, na.rm=TRUE), decreasing=FALSE)
xx <- xx[, oo, drop=FALSE]
## all drugs
pdf(file.path(saveres, "boxplot_ic50_cgp.pdf"), width=23, height=10)
par(las=3, mar=c(10, 4, 4, 2) + 0.1)
boxplot(xx, outline=FALSE, ylab="-log10(IC50)", main="Drug sensitivity\nCGP")
dev.off()
## drugs in common between CGP and CCLE
commoncell <- intersect(rownames(data.cgp), rownames(data.ccle))
commonix <- is.element(colnames(xx), drug.map2[ , "CGP"])
mycol <- rep("white", ncol(xx))
mycol[commonix] <- "red"
pdf(file.path(saveres, "boxplot_ic50_cgp_commondrugs.pdf"), width=15, height=7)
par(las=3, mar=c(5, 4, 4, 2) + 0.1, xaxt="n")
mp <- boxplot(xx[commoncell, , drop=FALSE], outline=FALSE, ylab="-log10(IC50)", main="Drugs sensitivity (IC50)\nCGP", col=mycol, cex=0.5)
axis(1, at=which(commonix), tick=TRUE, labels=T)
text(x=which(commonix) + 0.5, y=par("usr")[3] - (par("usr")[4] * 0.02), pos=2, labels=rownames(drug.map2), srt=45, xpd=NA, font=2, col="red", cex=0.75)
...
dimnames(auc.cgp) <- nn
xx <- auc.cgp
oo <- order(apply(auc.cgp, 2, median, na.rm=TRUE), apply(auc.cgp, 2, IQR, na.rm=TRUE), decreasing=FALSE)
xx <- xx[ , oo, drop=FALSE]
## boxplot for all drugs
pdf(file.path(saveres, "boxplot_auc_cgp.pdf"), width=23, height=10)
par(las=3, mar=c(10, 4, 4, 2) + 0.1)
boxplot(xx, outline=FALSE, ylab="AUC", main="Drug sensitivity (AUC)\nCGP")
dev.off()
## drugs in common between CGP and CCLE
commoncell <- intersect(rownames(data.cgp), rownames(data.ccle))
commonix <- is.element(colnames(xx), drug.map2[ , "CGP"])
mycol <- rep("white", ncol(xx))
mycol[commonix] <- "red"
pdf(file.path(saveres, "boxplot_auc_cgp_commondrugs.pdf"), width=15, height=7)
par(las=3, mar=c(5, 4, 4, 2) + 0.1, xaxt="n")
mp <- boxplot(xx[commoncell, , drop=FALSE], outline=FALSE, ylab="AUC", main="Drugs sensitivity (AUC)\nCGP", col=mycol, cex=0.5)
axis(1, at=which(commonix), tick=TRUE, labels=T)
text(x=which(commonix) + 0.5, y=par("usr")[3] - (par("usr")[4] * 0.02), pos=2, labels=rownames(drug.map2), srt=45, xpd=NA, font=2, col="red", cex=0.75)
# legend("topleft", legend=c("CGP", "CCLE"), col=c("lightgreen", "lightblue"), pch=15, bty="n")
dev.off()
...
## boxplot
xx <- -log10(ic50.ccle / 10^6)
oo <- order(apply(xx, 2, median, na.rm=TRUE), apply(xx, 2, IQR, na.rm=TRUE), decreasing=FALSE)
xx <- xx[ , oo, drop=FALSE]
## boxplot for all drugs
pdf(file.path(saveres, "boxplot_ic50_ccle.pdf"), width=23, height=10)
par(las=3, mar=c(10, 4, 4, 2) + 0.1)
boxplot(xx, outline=FALSE, ylab="-log10(IC50)", main="Drug sensitivity (IC50)\nCCLE")
dev.off()
## boxplot with drugs in common between CGP and CCLE
commoncell <- intersect(rownames(data.cgp), rownames(data.ccle))
commonix <- is.element(colnames(xx), drug.map2[ , "CCLE"])
mycol <- rep("white", ncol(xx))
mycol[commonix] <- "red"
mycol2 <- rep("black", ncol(xx))
mycol2[commonix] <- "red"
pdf(file.path(saveres, "boxplot_ic50_ccle_commondrugs.pdf"), width=15, height=7)
par(las=3, mar=c(5, 4, 4, 2) + 0.1, xaxt="n")
mp <- boxplot(xx[commoncell, , drop=FALSE], outline=FALSE, ylab="-log10(IC50)", main="Drugs sensitivity (IC50)\nCCLE", col=mycol, cex=0.5)
axis(1, at=which(commonix), tick=TRUE, labels=T)
text(x=1:ncol(xx) + 0.25, y=par("usr")[3] - (par("usr")[4] * 0.02), pos=2, labels=gsub("drugid_", "", colnames(xx)), srt=45, xpd=NA, font=2, col=mycol2, cex=0.75)
# legend("topleft", legend=c("CGP", "CCLE"), col=c("lightgreen", "lightblue"), pch=15, bty="n")
dev.off()
...
xx <- xx[ , oo, drop=FALSE]
## boxplot for all drugs
pdf(file.path(saveres, "boxplot_auc_ccle.pdf"), width=23, height=10)
par(las=3, mar=c(10, 4, 4, 2) + 0.1)
boxplot(xx, outline=FALSE, ylab="AUC", main="Drug sensitivity (AUC)\nCCLE")
dev.off()
## boxplot with drugs in common between CGP and CCLE
...
mycol2 <- rep("black", ncol(xx))
mycol2[commonix] <- "red"
pdf(file.path(saveres, "boxplot_auc_ccle_commondrugs.pdf"), width=15, height=7)
par(las=3, mar=c(5, 4, 4, 2) + 0.1, xaxt="n")
mp <- boxplot(xx[commoncell, , drop=FALSE], outline=FALSE, ylab="AUC", main="Drugs sensitivity (AUC)\nCCLE", col=mycol, cex=0.5)
axis(1, at=which(commonix), tick=TRUE, labels=T)
text(x=1:ncol(xx) + 0.25, y=par("usr")[3] - (par("usr")[4] * 0.02), pos=2, labels=gsub("drugid_", "", colnames(xx)), srt=45, xpd=NA, font=2, col=mycol2, cex=0.75)
...
## boxplot of drug sensitivity
## IC50
pdf(file.path(saveres, "boxplot_ic50_cgp_ccle.pdf"), width=9, height=6)
par(xaxt="n")
xx <- -log10(ic50.cgp[cell.common, drug.common, drop=FALSE] / 10^6)
colnames(xx) <- gsub("drugid_", "", colnames(xx))
yy <- -log10(ic50.ccle[cell.common, drug.common, drop=FALSE] / 10^6)
colnames(yy) <- gsub("drugid_", "", colnames(yy))
oo <- order(apply(xx, 2, median, na.rm=TRUE), decreasing=FALSE)
xx <- xx[ , oo, drop=FALSE]
yy <- yy[ , colnames(xx), drop=FALSE]
nn0 <- colnames(xx)
nn <- paste(rep(colnames(xx), each=3), c("CGP", "CCLE", "XXX"), sep=".")
nn[grep("*XXX", nn)] <- ""
zz <- matrix(NA, ncol=3 * ncol(xx), nrow=nrow(xx), dimnames=list(rownames(xx), nn))
zz[ , seq(1, ncol(zz), by=3)] <- xx
zz[ , seq(2, ncol(zz), by=3)] <- yy
mp <- boxplot(zz, outline=FALSE, horizontal=FALSE, col=rep(c("lightgreen", "lightblue", "white"), ncol(zz) / 3))
axis(1, at=1:length(mp$names), tick=TRUE, labels=T)
text(x=seq(1.5, ncol(zz), by=3) + 1.5, y=par("usr")[3] - (par("usr")[4] * 0.02), pos=2, labels=nn0, srt=45, xpd=NA, font=2, col="black")
legend("topleft", legend=c("CGP", "CCLE"), col=c("lightgreen", "lightblue"), pch=15, bty="n")
dev.off()
## AUC
## boxplot of drug sensitivity
pdf(file.path(saveres, "boxplot_auc_cgp_ccle.pdf"), width=9, height=6)
par(xaxt="n")
xx <- auc.cgp[cell.common, drug.common, drop=FALSE]
colnames(xx) <- gsub("drugid_", "", colnames(xx))
yy <- auc.ccle[cell.common, drug.common, drop=FALSE]
colnames(yy) <- gsub("drugid_", "", colnames(yy))
oo <- order(apply(xx, 2, median, na.rm=TRUE), decreasing=FALSE)
xx <- xx[ , oo, drop=FALSE]
yy <- yy[ , colnames(xx), drop=FALSE]
...
## keep only the common cell lines, drugs and genes
